...
import DB.Schema
import Types.Inventory (Inventory (..), MenuItem (..), MutationResponse (..), StrainLineage (..))
...
initializeDB DBConfig {..} = do
  let
    connSettings =
      [ ConnSetting.connection $
          ConnSetting.Conn.params
...
-- withConnection removed (Phase 3): was `withConnection pool f = f pool`,
-- a no-op wrapper with zero call sites.
