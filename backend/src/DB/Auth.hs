...
buildPHC salt h = T.concat
  [ "$argon2id$v=19$m="
  , T.pack (show (Argon2.memory     argonOpts))
  , ",t=", T.pack (show (Argon2.iterations  argonOpts))
  , ",p=", T.pack (show (Argon2.parallelism argonOpts))
  , "$", b64Enc salt
  , "$", b64Enc h
  ]
...
userRowToAuthUser row = AuthenticatedUser
  { auUserId     = userId row
...
lookupUserByUsername pool username = do
  userRows <- runSession pool $ Session.statement () $ run $ Rel8.select $ do
    u <- each userSchema
    where_ $ userName u ==. lit username
          &&. isActive u
    pure u
  case userRows of
    [u] -> pure (Just u)
...
createSession pool uid mRegisterId ua ip = do
  ...
  now    <- getCurrentTime
  raw    <- getEntropy 32
  let tokenText = encodeTokenBytes raw
  ...
  case decodeTokenText rawToken of
    ...
    Right raw -> do
      ...
      sessRows <- runSession pool $ Session.statement () $ run $ Rel8.select $ do
        ...
        where_ $ sessTokenHash sess ==. lit tHash
