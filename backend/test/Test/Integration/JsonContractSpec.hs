...
import Types.Auth (
  ...
  UserRole (Admin, Cashier, Customer, Manager),
  capabilitiesForRole,
 )
import Types.Inventory (
  Inventory (Inventory),
  ItemCategory (Edibles, Flower, PreRolls, Vaporizers),
  MenuItem (..),
  MutationResponse (MutationResponse),
  Species (Hybrid, Indica, IndicaDominantHybrid, Sativa),
  StrainLineage (
    StrainLineage,
    cbg,
    creator,
      ...
      it "sessionCapabilities is a nested object with capability flags" $ do
        let sess =
              SessionResponse
                ...
                , sessionRole = Admin
                , sessionCapabilities = capabilitiesForRole Admin
                }
        case toJSON sess of
