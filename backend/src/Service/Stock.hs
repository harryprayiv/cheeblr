...
transition pullId cmd applyResult = do
  ...
  let oldVertex    = toPullVertex someState
  ...
  case result of
    ...
    Right () -> do
      now <- currentTime
      emit $ StockEvt $ PullStatusChanged pullId oldVertex newVertex
               (actorFromCmd cmd) now
...
cancelPull pullId reason actorId = do
  (pr, someState) <- loadPull pullId
  let (evt, next) = runPullCommand someState (CancelCmd reason actorId)
      newVertex   = toPullVertex next
  guardPullEvent evt
  result <- updatePullStatus pullId newVertex Nothing
  case result of
...
addMessage pullId senderId fromRole msg = do
  _ <- loadPull pullId
  now <- currentTime
  let msgId = pullId -- will be replaced by GenUUID in wiring
      pm = PullMessage
        { pmId            = msgId
