...
import           Network.Wai           (Application, responseLBS,
                                         requestHeaders)
import           Network.Wai.Test      (defaultRequest, runSession,
                                         request, simpleHeaders)
import           Test.Hspec

import Server.Middleware.Tracing (tracingMiddleware, getTraceId)
...
spec = describe "Server.Middleware.Tracing" $ do
  ...
  describe "response header" $ do
    ...
    it "echoes incoming X-Trace-Id when valid" $ do
      let knownId = "11111111-1111-1111-1111-111111111111"
          req = defaultRequest
    ...
    it "two requests without trace ID get different IDs" $ do
      ...
      let h1 = lookup "X-Trace-Id" (simpleHeaders resp1)
