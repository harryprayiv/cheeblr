...
import           API.Manager
import           API.Transaction                 (DailyReportRequest (..),
                                                   DailyReportResult (..),
                                                   ComplianceReportRequest,
                                                   ComplianceReportResult (..),
                                                   Register (..))
import           Auth.Session                    (SessionContext (..), resolveSession)
import           Config.App                      (cfgLowStockThreshold,
...
buildAlerts env txs registers (Inventory invitems) = do
  now <- getCurrentTime
  let threshold = cfgLowStockThreshold (envConfig env)
...
activityHandler env mHeader = do
  ...
  let staleSecs  = cfgStaleTransactionSecs (envConfig env)
  ...
  case result of
    ...
    Right ctx ->
      if capCanViewReports (capabilitiesForRole (auRole (scUser ctx)))
        then filteredSseStream isManagerEvent
               (envDomainBroadcaster env) mCursor req sendResp
