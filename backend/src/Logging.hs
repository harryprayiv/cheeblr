...
mkPL ::
  ...
  Maybe Text -> -- details
  CheeblrPayload
mkPL uid role action res outcome amt dets =
  CheeblrPayload
    { plAction = action
...
logRegisterClose LogCtx {..} regId countedCash variance outcome =
  let sev = if variance /= 0 then WarningS else InfoS
   in emit
        ...
        ( "REGISTER CLOSED regId="
            <> showUUID regId
            <> " countedCash="
            <> fmtCents countedCash
            <> " variance="
            <> fmtCents variance
            <> " empId="
            <> lcUserId
