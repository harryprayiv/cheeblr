...
import Infrastructure.Broadcast (publish, subscribe, subChan)
...
updateAvailability stVar evt now = case evt of
  ...
  TransactionEvt (TransactionItemAdded { teItem = item }) -> do
    let skuId = TT.transactionItemMenuItemSku item
  ...
  where
    publishFor skuId = do
      st <- readTVar stVar
      pure $ fmap (\ai -> AvailabilityUpdate ai now)
